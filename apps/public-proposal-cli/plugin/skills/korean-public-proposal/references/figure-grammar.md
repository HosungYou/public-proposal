...
- Independent page QA requires actual final-render bytes, hash, canonical realpath, page locator, and a canonical KPP final-render receipt. The receipt binds source-document realpath/bytes, output realpath/bytes, trusted renderer executable realpath/bytes and version, and its own canonical receipt hash. QA rereads those paths (or uses the explicit test reader seam) before measuring the page. `renderFigureA4Page` creates fixtures only and cannot satisfy this boundary. For measurable SVG page renders QA then reads figure/image bindings, text and mark geometry, bounding boxes, collisions, and clipping directly from the render; a caller-authored layout status or fictitious page context is not evidence.
