...
The public `@longtable/public-proposal@0.2.3` package is available from npm. The Codex setup path is:

```bash
npx @longtable/public-proposal setup --provider codex
```

To reproduce this exact release, use `npx @longtable/public-proposal@0.2.3 ...`; leaving off the version resolves the current `latest` tag.
