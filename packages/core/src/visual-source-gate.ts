...
function revalidatePlannedFigure(value: unknown): SemanticFigureSpec {
  ...
  if (parsed.success) {
    const canonical = planFigure({
      ...
      semanticValueIntent: parsed.data.semanticValueIntent,
      decisionEffect: parsed.data.decisionEffect,
      nonDuplicateOf: parsed.data.nonDuplicateOf,
      encodedVariables: parsed.data.encodedVariables,
